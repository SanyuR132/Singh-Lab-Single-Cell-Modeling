...
from utilities import normalise_string
...
def _load_mtx(paths, orientation=None):
    values = readMtx(paths["values"]["full"])
    if orientation == "fbe":
        values = values.T
    val_shape = values.shape
    print("Num of cells = {} | Num of genes = {}".format(
        val_shape[0], val_shape[1]))
    example_names = numpy.array(["example {}".format(i+1)
                                for i in range(val_shape[0])]).flatten()
    feature_names = numpy.array(["feature {}".format(i+1)
                                for i in range(val_shape[1])]).flatten()
    labels = None
    # -----
    data_dictionary = {
        "values": values,
...
@_register_loader("h5ad_ebf")
def _load_ebf_h5ad_as_data_set(paths):
    return _load_h5ad(
        paths=paths,
        orientation="ebf"
    )
...
@_register_loader("h5ad_fbe")
def _load_fbe_h5ad_as_data_set(paths):
    return _load_h5ad(
        paths=paths,
        orientation="fbe"
    )
...
def _load_h5ad(paths, orientation=None):
    values = readH5ad(paths["values"]["full"])
    if orientation == "fbe":
        values = values.T
    val_shape = values.shape
    print("Num of cells = {} | Num of genes = {}".format(
