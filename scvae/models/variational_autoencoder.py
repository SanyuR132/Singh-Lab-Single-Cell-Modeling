...
import sys
sys.path.append(
    "/users/srajakum/vae_rnn/Structure_VAE_scRNA_Simulator/Models/scvae")
...
class VariationalAutoencoder:
    ...
    def train(self, training_set, validation_set=None, number_of_epochs=None,
        ...
        if not noisy_preprocess:
            ...
            print('training_set.values type:', type(training_set.values))
            print('training_set.preprocessed_values type:',
                  type(training_set.preprocessed_values))
        ...
        with tf.Session(graph=self.graph) as session:
            ...
            # )
            # with open(metadata_log_path, "wb") as metadata_log_file:
    ...
    def sample(self, sample_size=None, minibatch_size=None, run_id=None,
        ...
        print(f'sample size in sample method: {sample_size}')
    ...
    def train_and_evaluate(self, training_set, validation_set=None, number_of_epochs=None,
                           minibatch_size=None, learning_rate=None, run_id=None,
                           new_run=None, reset_training=None, **kwargs):
        if number_of_epochs is None:
            number_of_epochs = defaults["models"]["number_of_epochs"]
        ...
        with tf.Session(graph=self.graph) as session:
            ...
            x_eval = training_set.values
            if self.reconstruction_distribution_name == "bernoulli":
                t_eval = training_set.binarised_values
                evaluation_set_transformed = True
            else:
                t_eval = training_set.values
            output_versions = ["transformed", "reconstructed", "latent"]
            evaluation_subset_indices = set()
            n_examples_eval = training_set.number_of_examples
            n_features_eval = training_set.number_of_features
            if "reconstructed" in output_versions:
                p_x_mean_eval = numpy.empty(
                    shape=(n_examples_eval, n_features_eval),
                    dtype=numpy.float32
            ...
            print()
            # Data sets
            output_sets = [None, None, None]
            evaluation_set_transformed = False
            if "transformed" in output_versions:
                if evaluation_set_transformed:
                    transformed_evaluation_set = DataSet(
                        training_set.name,
                        title=training_set.title,
                        specifications=training_set.specifications,
                        values=t_eval,
                        preprocessed_values=None,
                        labels=training_set.labels,
                        example_names=training_set.example_names,
                        feature_names=training_set.feature_names,
                        batch_indices=training_set.batch_indices,
                        batch_names=training_set.batch_names,
                        features_mapped=training_set.features_mapped,
                        feature_selection=training_set.feature_selection,
                        example_filter=training_set.example_filter,
                        preprocessing_methods=(
                            training_set.preprocessing_methods),
                        kind=training_set.kind,
                        version="transformed"
                    )
                else:
                    transformed_evaluation_set = training_set
                # index = output_versions.index("transformed")
                output_sets[0] = transformed_evaluation_set
            ...
            if "reconstructed" in output_versions:
                reconstructed_evaluation_set = DataSet(
                    ...
                    feature_selection=training_set.feature_selection,
                    example_filter=training_set.example_filter,
                    preprocessing_methods=training_set.preprocessing_methods,
                    kind=training_set.kind,
                    version="reconstructed"
                )
                # index = output_versions.index("reconstructed")
                output_sets[1] = reconstructed_evaluation_set
            ...
            if "latent" in output_versions:
                ...
                output_sets[2] = latent_evaluation_sets
            print()
            ...
            return output_sets, permanent_log_directory
